...
- `VITE_AUTH_ROUTE_MODE` 同一时间只生效一种，默认 `static`：不请求菜单接口，侧边栏从本地 `(admin)` 文件树的 `staticData` 生成；没有 `staticData` 的节点丢弃，也不会把子路由提到上一级。
- 若改为 `dynamic`：登录后拉当前账号菜单，本地文件树没有的 path 整节点（含 children）丢弃。后端菜单只授权本地 route tree，忽略 `element`。后端 `:param` 会先转为 `$param`。`redirect` 不进入授权集合，由文件路由 `beforeLoad` 跳转。字段对应见 `docs/ROUTING.md`。
...
- `(admin)/layout` `beforeLoad: guardAdminRoute`：无 token → `/login`（`/home` 不带 redirect）；用户空则 `revokeSession`，由 Guard 唯一负责 redirect。默认 static：本地文件树没有这条 `originPath` → 404，有则放行。若改为 dynamic：当前账号菜单未包含该 `originPath` 时，本地有路由 → `/403`，本地也没有 → 404。未知 URL 由 root 404。授权 identity 为 `matches.at(-1).fullPath`。
...
- 默认 static：keepAlive / multi / activeMenu 来自本地 `staticData`，Tab 与当前页 pane 同源。若改为 dynamic：这三项来自当前账号菜单的 `handle`（与 `meta` 等价）；Tab 的 keepAlive 以菜单项为准（显式 `false` 不会被本地 `true` 覆盖），停在当前页时 pane 额外看 `route.staticData.keepAlive`。后端不加载 `element`。
