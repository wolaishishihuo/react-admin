...
- 主题色默认 `#B40006`（用户可换，派生 `colorPrimary5/8` 两档供 ThemeDrawer 预览）。
...
- 主题：`themeMode` 三档 light/dark/auto，默认 **light**；auto 由 `matchMedia('(prefers-color-scheme)')` 实时跟随系统；用户选择持久化，实际生效读 `isDark`；`index.html` 内联脚本读 `theme-state` 做首屏防闪（含 auto 档）。水印默认关闭。
