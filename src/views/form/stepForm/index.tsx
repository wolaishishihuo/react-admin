...
const SecondContent = ({ prev, next }: any) => {
  ...
  return (
    <Form name="second" layout="vertical" className="second-form" form={form} onFinish={onFinish} onFinishFailed={onFinishFailed}>
      <Form.Item name="username" hidden initialValue="ant-design@alipay.com">
        <Input autoComplete="username" />
      </Form.Item>
      <Alert title="确认转账后，资金将直接打入对方账户，无法退回。" type="info" showIcon />
      ...
      <Form.Item label="支付密码" name="password" rules={[{ required: true, message: "需要支付密码才能进行支付" }]}>
        <Input.Password placeholder="请输入支付密码" autoComplete="new-password" />
      </Form.Item>
      <Form.Item>
        <Space>
...
const LastContent = ({ setCurrent }: any) => {
  return (
    <Result
      ...
      extra={[
        <Button type="primary" key="retry" onClick={() => setCurrent(0)}>
          再转一笔
        </Button>
      ]}
    >
...
const StepForm: React.FC = () => {
  ...
  const steps = [{ title: "填写转账信息" }, { title: "确认转账信息" }, { title: "完成" }];

  const contents = [
    <FirstContent key="first" next={next} />,
    <SecondContent key="second" prev={prev} next={next} />,
    <LastContent key="last" setCurrent={setCurrent} />
  ];
  ...
  return (
    <React.Fragment>
      ...
      <Card className="setup-form">
        ...
        <div className="setup-form-content">{contents[current]}</div>
      </Card>
    </React.Fragment>
  );
};
