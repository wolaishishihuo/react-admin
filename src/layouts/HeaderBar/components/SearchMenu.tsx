...
import { flattenMenu } from '@/features/navigation/menu-tree';
import type { NavigationItem } from '@/features/navigation/types';
import { navigateTo } from '@/router/router-ref';
import { addSearchHistory, removeSearchHistory, useSearchHistoryStore } from '@/stores/modules/search-history.store';
...
const SearchMenu: React.FC = () => {
  ...
  const flatMenuList = useMemo(() => flattenMenu(tree), [tree]);
  ...
  const mouseoverMenuItem = (item: NavigationItem) => {
  ...
  const selectMenuItem = (menu: NavigationItem) => {
    addSearchHistory(menu.path);
    if (menu.external && isHttpUrl(menu.external)) {
      openExternal(menu.external);
      closeModal();
      return;
    }
    void navigateTo(menu.path);
    closeModal();
  };
  ...
  const handleDeleteHistory = (event: React.MouseEvent, item: NavigationItem) => {
        ...
        {displayList.length > 0 && (
          <div className={clsx('menu-list max-h-510px overflow-auto', isHistoryMode ? 'mt-5px' : 'mt-15px')} ref={menuListRef}>
            {displayList.map(item => (
              ...
              >
                <Icon className={clsx('menu-icon text-16px mr-8px', item.path === activePath && 'text-18px')} name={item.icon!} />
                <span className={clsx('menu-title text-14px', item.path === activePath && 'text-16px')}>{item.title}</span>
                {isHistoryMode ? (
                  <span
